...
- **OpenList/AList 视频播放**：粘贴 OpenList 实例链接（`https://实例/d/视频.mp4`）即可播放——服务端会自动调用该实例 API 解析出**云盘直链**（二次地址）供播放器播放，而**视频码（vid）与字幕检测仍以原始 OpenList 链接为准**：直链带签名/会过期也不影响弹幕与字幕关联。解析失败的链接原样播放；未配置实例的链接不受影响
- **OpenList/AList 同目录字幕**：视频 URL 属于已配置的 OpenList 云端实例（`/d/` 路径）时，自动用该实例 API 列出视频同目录的字幕文件（`视频.sc.srt` 等）并作为字幕选项
- 删除字幕时自动解除所有视频关联
