...
- **Multi-subtitle detection**: auto-detects `.srt/.vtt/.ass` files next to the video, grouped by language (SC/TC/EN/JA/KO...), switchable in the player`n- **Subtitle library**: a dedicated Subtitles tab with its own IDs — add subtitles from URL (one-click localize), pasted text, or uploaded files; apply one or more subtitles to any video (videos still get same-directory auto-detection merged in); Paste an OpenList instance link (https://instance/d/video.mp4, optionally with `?sign=`): on playback the server immediately converts it to the **cloud direct link** via the instance API — no browser-side redirect. Resolution failures play the original link as-is; links from unconfigured instances are unaffected. The **video ID and subtitle associations are keyed to the normalized link (signature stripped)**, so expiring/changing signatures never cause vid drift and historical danmaku/subtitles stay attached. Same-folder subtitles (`video.sc.srt` etc.) are offered as options too; player supports `subtitle=id:xxxx`
