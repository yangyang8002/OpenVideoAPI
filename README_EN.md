...
- **Multi-subtitle detection**: auto-detects `.srt/.vtt/.ass` files next to the video, grouped by language (SC/TC/EN/JA/KO...), switchable in the player`n- **Subtitle library**: a dedicated Subtitles tab with its own IDs — add subtitles from URL (one-click localize), pasted text, or uploaded files; apply one or more subtitles to any video (videos still get same-directory auto-detection merged in); Paste an OpenList instance link (https://instance/d/video.mp4) to play — the server auto-resolves the cloud direct link (secondary URL) via the instance API, while the video ID and subtitle detection stay keyed to the original OpenList link (signed/expiring direct links never affect danmaku or subtitles). Links that fail to resolve play as-is; links from unconfigured instances are unaffected. OpenList/AList cloud videos also get same-folder subtitle detection via the configured instance API; player supports `subtitle=id:xxxx`
